...
3. Production database is MySQL 8.4 LTS with InnoDB, utf8mb4, UTC storage, foreign keys, and named constraints. MySQL 8.0 reached end of life in April 2026 and is migration-source compatibility only, not the production target. **UTC storage is settled and normative — owner decision 2026-08-15.** The application stores UTC (`TIMEZONE=UTC` in `.env`, `'timezone' => '+00:00'` on the MySQL connection in `config/database.php`) and renders user-facing times in the branch timezone, default `Asia/Taipei`, at the display layer only. This agrees with `PRD.md` L859, L877, L909 and L1502; the earlier `Asia/Taipei`-at-rest proposal was considered and **rejected**, and the whole comparison is recorded in `UPSTREAM.md` **UP-009**. Read that entry before changing the timezone in either direction. **Known consequence, accepted:** `event_attendance_sessions.attendance_date` is a `date()` column that never converts, so a service held between 00:00 and 08:00 Taipei records the previous UTC calendar day. Any code deriving a calendar day from an instant must convert to the branch timezone first; this is characterized, not left implicit.
